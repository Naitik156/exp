...
const StopwatchView = () => {
    ...
    const stopwatchRef = React.useRef(null); // Fullscreen element ref - button clicks inside won't exit fullscreen
    ...
    const toggleFullScreen = () => {
        const el = stopwatchRef.current;
        if (!document.fullscreenElement) {
            // Request fullscreen on the stopwatch div itself, NOT document.documentElement
            // This way button clicks INSIDE the fullscreen element don't trigger exit
            const fsRequest = el 
                ? (el.requestFullscreen || el.webkitRequestFullscreen || el.mozRequestFullScreen || el.msRequestFullscreen)
                : null;
            if (fsRequest && el) {
                fsRequest.call(el).catch(e => {
                    // Fallback: if element fullscreen fails, try document
            ...
            if (fsExit) fsExit.call(document).catch(() => {});
    ...
    useEffect(() => {
        // Sync isFocusMode with actual fullscreen state
        // This handles ESC key press and other external fullscreen exits
...
// --- FINAL CSS: SCALING LOGIC (VMIN + Font Fix) ---
    ...
    return React.createElement('div', { 
        ...
        React.createElement('div', { className: 'stopwatch-container' },
            ...
            // CONTROLS — all buttons have stopPropagation + preventDefault to prevent fullscreen exit on mobile
            React.createElement('div', { 
                ...
                !isRunning 
                    ? React.createElement('button', { 
                        className: 'btn-circle btn-start', 
                        onClick: (e) => { e.stopPropagation(); e.preventDefault(); handleStart(e); },
                        onTouchEnd: (e) => { e.stopPropagation(); e.preventDefault(); handleStart(e); },
                        onTouchStart: (e) => e.stopPropagation()
                    }, 'Start') 
                    : React.createElement('button', { 
                        className: 'btn-circle btn-stop', 
                        onClick: (e) => { e.stopPropagation(); e.preventDefault(); handleStop(e); },
                        onTouchEnd: (e) => { e.stopPropagation(); e.preventDefault(); handleStop(e); },
                        onTouchStart: (e) => e.stopPropagation()
                    }, 'Pause'),
                React.createElement('button', { 
                    className: 'btn-circle btn-reset', 
                    onClick: (e) => { e.stopPropagation(); e.preventDefault(); handleReset(e); },
                    onTouchEnd: (e) => { e.stopPropagation(); e.preventDefault(); handleReset(e); },
                    onTouchStart: (e) => e.stopPropagation()
                }, 'Reset'),
                (isRunning || elapsed > 0) && React.createElement('button', { 
                    className: 'btn-circle btn-lap', 
                    onClick: (e) => { e.stopPropagation(); e.preventDefault(); handleLap(e); },
                    onTouchEnd: (e) => { e.stopPropagation(); e.preventDefault(); handleLap(e); },
                    onTouchStart: (e) => e.stopPropagation()
                }, 'Lap')
            ),
