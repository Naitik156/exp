...
const StopwatchView = () => {
    ...
    // --- FULLSCREEN TOGGLE ---
    // We use CSS-only "fullscreen" (position:fixed, 100vw x 100vh, z-index:9999)
    // instead of the browser Fullscreen API (requestFullscreen).
    // Reason: Browser Fullscreen API on mobile Chrome/Android exits fullscreen
    // whenever ANY button click causes a React state update + DOM re-render,
    // because the browser interprets DOM mutations as "user interaction outside
    // the fullscreen element" and auto-exits. CSS-only approach has ZERO such issues.
    // ESC key is handled by our own keydown listener below.
    const toggleFullScreen = () => {
        setIsFocusMode(prev => !prev);
    };

    // Allow ESC key to exit CSS fullscreen mode
    useEffect(() => {
    ...
    // Lock/restore body scroll when CSS fullscreen is active
    useEffect(() => {
        if (isFocusMode) {
...
// --- FINAL CSS: SCALING LOGIC (VMIN + Font Fix) ---
    const fixedStyles = `
        ...
        /* 3. CSS FULLSCREEN MODE - position:fixed covers entire viewport, no browser API needed */
        /* This approach is immune to button-click-exits that plague requestFullscreen() on mobile */
    ...
    return React.createElement('div', { 
        ...
        isFocusMode && React.createElement('button', { 
            ...
            style: { position: 'absolute', top: '20px', right: '30px', background: 'none', border: 'none', color: '#444', fontSize: '1.5rem', cursor: 'pointer', zIndex: 10000 } 
        }, '⛶'),
        ...
        React.createElement('div', { className: 'stopwatch-container' },
            ...
            // CONTROLS - simple onClick handlers, no fullscreen API to worry about
