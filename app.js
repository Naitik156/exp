        ...
        if (user && window.db && window.dbFuncs) {
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    // --- BUG FIX: currentExam restore karo after logout+login ---
                    // Pehle localStorage check, fir Firestore (_currentExam field) se restore
                    const lsExam = localStorage.getItem('currentExam');
                    const firestoreExam = loadedData?._currentExam || '';
                    const restoredExam = (lsExam && lsExam !== '') ? lsExam : (firestoreExam !== '' ? firestoreExam : null);
                    if (restoredExam) {
                        setCurrentExam(restoredExam);
                        localStorage.setItem('currentExam', restoredExam);
                        // Agar view abhi bhi exam-select par hai toh home par le jao
                ...
                try {
                    ...
                    if (backup) {
                        ...
                        setData(parsed);
                        // Offline mein bhi currentExam restore karo
                        const lsExam = localStorage.getItem('currentExam');
                        const firestoreExam = parsed?._currentExam || '';
                        const restoredExam = (lsExam && lsExam !== '') ? lsExam : (firestoreExam !== '' ? firestoreExam : null);
                        if (restoredExam) {
                            setCurrentExam(restoredExam);
                            setView(prev => (prev === 'exam-select' ? 'home' : prev));
