            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    const safeData = {
                        ...
                        syllabusCustom: loadedData?.syllabusCustom || {}
                    };
                    setData(safeData);
...
// --- INSTANT SAVE FIX (Reload karne par data nahi udega) ---
    ...
    // getChapters: merges base EXAM_SYLLABUS with persisted syllabusCustom changes
    const getChapters = (className, subject) => {
        const base = EXAM_SYLLABUS[currentExam]?.[className]?.[subject] || [];
        const custom = data.syllabusCustom?.[currentExam]?.[className]?.[subject] || {};
        const added   = custom.added   || [];
        const deleted = custom.deleted || [];
        const renamed = custom.renamed || {};
        const result = base
            .map(ch => renamed[ch] !== undefined ? renamed[ch] : ch)
            .filter(ch => {
                const orig = Object.keys(renamed).find(k => renamed[k] === ch) || ch;
                return !deleted.includes(orig) && ch !== '';
            });
        added.forEach(ch => { if (ch && !result.includes(ch)) result.push(ch); });
        return result;
    };

    // saveSyllabusCustom: saves new syllabusCustom to state + Firestore instantly
    const saveSyllabusCustom = (nextCustom) => {
        setData(prev => {
            const updated = { ...prev, syllabusCustom: nextCustom };
            if (user && window.db && window.dbFuncs) {
                const { doc, setDoc } = window.dbFuncs;
                setDoc(doc(window.db, "users", user.uid), updated, { merge: true })
                    .catch(err => console.log("SyllabusCustom Save Error:", err));
            }
            return updated;
        });
    };
    ...
    const addCustomChapter = (className, subject, chapterName) => {
        const prev = data.syllabusCustom || {};
        const examC = prev[currentExam] || {};
        const classC = examC[className] || {};
        const subjC = classC[subject] || {};
        const added = subjC.added || [];
        const currentChapters = getChapters(className, subject);
        if (!currentChapters.includes(chapterName) && !added.includes(chapterName)) {
            const nextCustom = {
                ...prev,
                [currentExam]: {
                    ...examC,
                    [className]: {
                        ...classC,
                        [subject]: { ...subjC, added: [...added, chapterName] }
                    }
                }
            };
            saveSyllabusCustom(nextCustom);
    ...
    const handleDeleteChapter = (className, subject, chapter) => {
        setModalConfig({
            ...
            onConfirm: () => {
                ...
                const subjC = classC[subject] || {};
                const added   = subjC.added   || [];
                const deleted = subjC.deleted || [];
                const renamed = subjC.renamed || {};
                const origName = Object.keys(renamed).find(k => renamed[k] === chapter) || chapter;
                const isCustomAdded = added.includes(chapter);
                let newAdded   = isCustomAdded ? added.filter(c => c !== chapter) : added;
                let newDeleted = isCustomAdded ? deleted : (deleted.includes(origName) ? deleted : [...deleted, origName]);
                let newRenamed = { ...renamed };
                if (!isCustomAdded) delete newRenamed[origName];
                const nextCustom = {
                    ...prev,
                    [currentExam]: {
                        ...examC,
                        [className]: {
                            ...classC,
                            [subject]: { ...subjC, added: newAdded, deleted: newDeleted, renamed: newRenamed }
                        }
                    }
                };
                setData(p => {
                    const copy = { ...p, syllabusCustom: nextCustom };
                    if (copy[currentExam]?.[className]?.[subject]) {
                        const subCopy = { ...copy[currentExam][className][subject] };
                        delete subCopy[chapter];
                        copy[currentExam] = { ...copy[currentExam], [className]: { ...copy[currentExam][className], [subject]: subCopy } };
                    }
                    if (user && window.db && window.dbFuncs) {
                        const { doc, setDoc } = window.dbFuncs;
                        setDoc(doc(window.db, "users", user.uid), copy, { merge: true })
                            .catch(err => console.log("Delete Save Error:", err));
                    }
                    return copy;
                });
                setShowModal(false);
                showToast(`Chapter deleted: ${chapter}`);
            }
        });
        setShowModal(true);
    };
...
const getAnalytics = (filterClass = 'Overall') => {
        ...
        reader.onload = (e) => {
            try {
                ...
                const safeData = {
                    ...
                    syllabusCustom: importedData?.syllabusCustom || {}
                };
                setData(safeData);
        ...
        return React.createElement('div', { className: 'container' },
            ...
            React.createElement('div', { className: 'grid grid-3' },
                subjects.map(subject => React.createElement('div', {
                    ...
                    React.createElement('p', { className: 'card-subtitle' }, `${getChapters(selectedClass, subject).length} Chapters`),
                    React.createElement('div', { className: 'progress-container', style: { marginTop: '1.5rem' } },
                        React.createElement('div', { className: 'progress-label' },
    ...
    const ChaptersView = () => {
        const chapters = getChapters(selectedClass, selectedSubject);
        ...
        const handleRenameChapter = (oldName) => {
            if (editedName.trim() && editedName.trim() !== oldName) {
                ...
                const prev = data.syllabusCustom || {};
                const examC = prev[currentExam] || {};
                const classC = examC[selectedClass] || {};
                const subjC = classC[selectedSubject] || {};
                const added   = subjC.added   || [];
                const renamed = subjC.renamed || {};
                const origName = Object.keys(renamed).find(k => renamed[k] === oldName) || oldName;
                const isCustomAdded = added.includes(oldName);
                let newAdded   = isCustomAdded ? added.map(c => c === oldName ? trimmedName : c) : added;
                let newRenamed = { ...renamed };
                if (!isCustomAdded) newRenamed[origName] = trimmedName;
                const nextCustom = {
                    ...prev,
                    [currentExam]: {
                        ...examC,
                        [selectedClass]: {
                            ...classC,
                            [selectedSubject]: { ...subjC, added: newAdded, renamed: newRenamed }
                        }
                    }
                };
                const oldChapterData = getChapterData(selectedClass, selectedSubject, oldName);
                setData(p => {
                    const copy = { ...p, syllabusCustom: nextCustom };
                    if (copy[currentExam]?.[selectedClass]?.[selectedSubject]) {
                        const subCopy = { ...copy[currentExam][selectedClass][selectedSubject] };
                        if (Object.keys(oldChapterData).length > 0) subCopy[trimmedName] = { ...oldChapterData };
                        delete subCopy[oldName];
                        copy[currentExam] = { ...copy[currentExam], [selectedClass]: { ...copy[currentExam][selectedClass], [selectedSubject]: subCopy } };
                    }
                    if (user && window.db && window.dbFuncs) {
                        const { doc, setDoc } = window.dbFuncs;
                        setDoc(doc(window.db, "users", user.uid), copy, { merge: true })
                            .catch(err => console.log("Rename Save Error:", err));
                    }
                    return copy;
                });
                showToast(`Renamed: ${oldName} → ${trimmedName}`);
            }
            setEditingChapter(null);
            setEditedName('');
        };
const handleDeleteChapter = (chapterName) => {
    // Delegates to App-level handleDeleteChapter which uses syllabusCustom persistence
    handleDeleteChapter(selectedClass, selectedSubject, chapterName);
};
