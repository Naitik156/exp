...
// --- INSTANT SAVE FIX (Reload karne par data nahi udega) ---
    ...
    setModalConfig({
        ...
        onConfirm: () => {
            ...
            setShowModal(false);
            forceChapterRefresh(); // Trigger re-render after mutation
        ...
        onConfirm: () => {
            ...
            setData(prev => {
                ...
                if (copy[currentExam]?.[selectedClass]?.[selectedSubject]) {
                    ...
                    delete subCopy[chapterName];
                    copy[currentExam] = {
                        ...copy[currentExam],
                        [selectedClass]: {
                            ...copy[currentExam][selectedClass],
                            [selectedSubject]: subCopy
                        }
                    };
                }
                return copy;
...
const StopwatchView = () => {
    ...
    const toggleFullScreen = () => {
        // CSS-only fullscreen: just toggle state, no browser Fullscreen API.
        // requestFullscreen() on mobile Chrome auto-exits whenever a button click
        // triggers setData() -> React re-render -> DOM mutation.
        // position:fixed via CSS is 100% immune to this problem.
        setIsFocusMode(prev => !prev);
    };
    
    // UI Render part (Keep it as it was in original code)...

    // Lock body scroll when CSS-fullscreen is active, restore on exit
    useEffect(() => {
        if (isFocusMode) {
    ...
    // Allow ESC key to exit CSS fullscreen
    useEffect(() => {
...
// --- FINAL CSS: SCALING LOGIC (VMIN + Font Fix) ---
    const fixedStyles = `
        ...
        /* 3. FULLSCREEN MODE - position:fixed, no browser Fullscreen API needed */
