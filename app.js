...
const [selectedChapter, setSelectedChapter] = useState(() => localStorage.getItem('lastChapter') || null);
    ...
    const [loadError, setLoadError] = useState(null); // null | 'offline' | 'firebase' | 'timeout'
    const [saveStatus, setSaveStatus] = useState('idle'); // 'idle' | 'saving' | 'saved' | 'error'
    // --- ONLINE/OFFLINE DETECTION ---
    useEffect(() => {
        const handleOnline = () => {
            setIsOnline(true);
            // If we had a load error due to offline, retry loading data
            if (loadError === 'offline' && user) {
                setLoadError(null);
                setIsFetched(false);
            }
        ...
        const handleOffline = () => {
            setIsOnline(false);
            showToast('⚠️ You are offline. Data will be saved locally.');
        };
...
useEffect(() => {
    const loadFromDB = async () => {
        if (user && window.db && window.dbFuncs) {
            // Reset error state on fresh load attempt
            setLoadError(null);

            // Timeout: agar 12 seconds mein Firebase respond nahi kiya
            ...
            try {
                ...
                // Detect if offline
                const errorType = !navigator.onLine ? 'offline' : 'firebase';
                ...
                try {
                    ...
                    if (backup) {
                        ...
                        setIsFetched(true);
                        setLoadError(null); // Cache se data mila, koi error nahi dikhana
                    } else {
                        // No backup either
                        setIsFetched(true);
                    ...
                    console.error("Backup load error:", backupErr); 
                    setIsFetched(true);
                    setLoadError(errorType);
                }
            }
        ...
        const saveToDB = async () => {
            if (user && isFetched && window.dbFuncs && window.db) {
                ...
                try {
                    ...
                    try { localStorage.setItem('localDataBackup_' + user.uid, JSON.stringify(data)); } catch(e) {}
                    // Show save error toast only for non-offline errors
                    if (navigator.onLine) {
    ...
    const addCustomChapter = (className, subject, chapterName) => {
        ...
        if (!currentChapters.includes(chapterName)) {
            setData(prev => {
                ...
                if (user && window.db && window.dbFuncs) {
                    const { doc, setDoc } = window.dbFuncs;
                    setDoc(doc(window.db, "users", user.uid), nextState, { merge: true }).catch(err => console.log("Save Error:", err));
                }
                return nextState;
            });
...
filtered.length === 0 ? React.createElement('p', {style:{textAlign:'center', padding:'2rem'}}, 'No mistakes found.') : 
    ...
    // But agar load error hai toh error screen dikhao
    if (!isFetched && loadError) {
        return React.createElement('div', { 
            style: { 
            ...
            React.createElement('div', { 
                style: { 
                    background: 'white', borderRadius: '24px', padding: '2.5rem', maxWidth: '380px', width: '100%',
                    boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)', border: '1px solid rgba(255,255,255,0.3)'
                }
            },
                React.createElement('div', { style: { fontSize: '3rem', marginBottom: '1rem' } }, 
                    loadError === 'offline' ? '📡' : loadError === 'timeout' ? '⏱️' : '🔥'
                ),
                React.createElement('h2', { style: { fontFamily: "'DM Serif Display', serif", color: '#1C1917', marginBottom: '0.75rem' } },
                    loadError === 'offline' ? 'No Internet Connection' : 
                    loadError === 'timeout' ? 'Loading Timed Out' : 
                    'Connection Error'
                ),
                React.createElement('p', { style: { color: '#57534E', marginBottom: '1.5rem', lineHeight: '1.6', fontSize: '0.9rem' } },
                    loadError === 'offline' ? 'Please connect to the internet and try again. Your cached data is safe.' :
                    loadError === 'timeout' ? 'Server is taking too long to respond. Please check your connection and retry.' :
                ...
                React.createElement('button', { 
                    ...
                    onClick: () => { setLoadError(null); setIsFetched(false); if (user) { setIsFetched(false); } window.location.reload(); }
                }, '🔄 Retry'),
                React.createElement('button', { 
                    style: { 
                        width: '100%', padding: '12px', borderRadius: '12px', border: '1.5px solid #E7E5E4',
                        background: 'white', cursor: 'pointer', color: '#57534E', fontWeight: '600', fontSize: '0.9rem'
                    },
   ...
   return React.createElement(React.Fragment, null,
        ...
        // --- 5. OFFLINE BANNER (Persistent when no internet) ---
        !isOnline && React.createElement('div', { 
            ...
            React.createElement('span', null, '📡'),
            React.createElement('span', null, 'Aap offline hain — Data locally save ho raha hai. Internet aate hi sync hoga.')
        ),

        // --- 6. SAVE STATUS INDICATOR ---
        (saveStatus === 'saving' || saveStatus === 'error') && React.createElement('div', { 
            style: { 
                position: 'fixed', bottom: '80px', right: '16px', zIndex: 9990,
                background: saveStatus === 'error' ? '#FEF2F2' : '#F0FDF4',
                border: `1.5px solid ${saveStatus === 'error' ? '#FECACA' : '#BBF7D0'}`,
                ...
                ? React.createElement(React.Fragment, null,
                    React.createElement('span', { 
                        style: { 
                            ...
                            border: '2px solid rgba(21,128,61,0.3)', borderLeft: '2px solid #15803D',
