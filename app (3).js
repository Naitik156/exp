...
// --- Global ads injected once (Social Bar only) ---
// Popunder is now SMART — only fires on background→foreground, never on first visit
const _adsGlobalInjected = { socialBar: false };

const injectGlobalAds = () => {
    // Social Bar — inject once for all users
    if (!_adsGlobalInjected.socialBar) {
        _adsGlobalInjected.socialBar = true;
        const s = document.createElement('script');
        s.src = 'https://pl28761534.effectivegatecpm.com/f8/46/a9/f846a9ea84e26c5dbe3c5281e54d771b.js';
        s.async = true;
        document.body.appendChild(s);
    }
};

// ─── SMART POPUNDER SYSTEM ────────────────────────────────────────────────────
// Rules:
//   1. NEVER on very first visit ever (pehli baar aaya = no ad, good impression)
//   2. Only fires when user comes BACK from background (minimize/tab switch)
//   3. User must have spent > 2 min in app (serious user hai)
//   4. MAX 2 times per day
//   5. Same for new & old users — bas first-ever visit pe nahi
// ─────────────────────────────────────────────────────────────────────────────
const _popunderState = {
    sessionStart: Date.now(),
    wentBackground: false,
    firedThisSession: false
};

const _canFirePopunder = () => {
    const TODAY = new Date().toLocaleDateString('en-CA');
    const countKey = 'pdCount_' + TODAY;
    const count = parseInt(localStorage.getItem(countKey) || '0', 10);
    const timeSpent = (Date.now() - _popunderState.sessionStart) / 1000;

    if (!localStorage.getItem('hasVisitedBefore')) return false; // First ever visit
    if (count >= 2) return false;                                  // Max 2/day
    if (timeSpent < 120) return false;                             // < 2 min spent
    if (_popunderState.firedThisSession) return false;             // Once per session
    if (!_popunderState.wentBackground) return false;              // Must come from bg
    return true;
};

const _firePopunder = () => {
    if (!_canFirePopunder()) return;
    _popunderState.firedThisSession = true;
    _popunderState.wentBackground = false;
    const TODAY = new Date().toLocaleDateString('en-CA');
    const countKey = 'pdCount_' + TODAY;
    localStorage.setItem(countKey, String(parseInt(localStorage.getItem(countKey) || '0', 10) + 1));
    // Inject popunder script
    const s = document.createElement('script');
    s.src = 'https://pl28762670.effectivegatecpm.com/a3/f1/c7/a3f1c71b27f6dc99a370605df6800967.js';
    s.async = true;
    document.body.appendChild(s);
};

// Listen for background → foreground transitions
(function _setupPopunderListener() {
    // Mark first visit
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'hidden') {
            _popunderState.wentBackground = true;
        } else if (document.visibilityState === 'visible' && _popunderState.wentBackground) {
            // Small delay so page renders before ad fires
            setTimeout(_firePopunder, 800);
        }
    });
})();

// --- Native Banner Ad Component ---
// FIX: Adsterra Native Banner needs a REAL DOM div with correct container ID.
// We create an isolated div OUTSIDE React tree using a portal-like approach.
const NATIVE_BANNER_SCRIPT = 'https://pl28761568.effectivegatecpm.com/43d2829f3679a811b6e55ce7e306e315/invoke.js';
const NATIVE_BANNER_CONTAINER = 'container-43d2829f3679a811b6e55ce7e306e315';

// Track how many ad slots have been injected
let _nbInjectedCount = 0;
...
const AdBanner = () => {
    ...
    useEffect(() => {
        ...
        _nbInjectedCount++;
        const slotNum = _nbInjectedCount;

        // Create the ad div with exact Adsterra container ID
        // For multiple slots: first uses original ID, extras use unique IDs
        const containerId = slotNum === 1
            ? NATIVE_BANNER_CONTAINER
            : NATIVE_BANNER_CONTAINER + '-' + slotNum;

        const adDiv = document.createElement('div');
        adDiv.id = containerId;
        adDiv.style.cssText = 'width:100%;min-height:60px;display:block;';
        hostRef.current.appendChild(adDiv);

        // Only inject the script once (first slot) — it auto-fills all containers
        if (slotNum === 1) {
            const s = document.createElement('script');
            s.async = true;
            s.setAttribute('data-cfasync', 'false');
            s.src = NATIVE_BANNER_SCRIPT;
...
const HomePage = (ctx) => {
    ...
    return React.createElement('div', { className: 'container' },
        ...
        React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', gap: '10px' } },
            ...
            // --- NATIVE BANNER AD (Class cards ke baad, feature cards se pehle) ---
            React.createElement(AdBanner, { key: 'home-ad-1' }),
...
const SubjectsView = (ctx) => {
    const { data, setData, currentExam, setCurrentExam, user, navigateTo, changeExam, showToast, setShowModal, setModalConfig, getChapterData, getProgress, getSubjectProgress, getClassProgress, getAnalytics, getChapters, getSubjectClass, updateChapterData, addCustomChapter, exportData, importData, selectedClass, setSelectedClass, selectedSubject, setSelectedSubject, selectedChapter, setSelectedChapter, editMode, setEditMode, isFocusMode, setIsFocusMode, forceChapterRefresh, unsavedChangesRef, isFetched, customChapterVersion } = ctx;
        ...
        return React.createElement('div', { className: 'container' },
            ...
            // --- NATIVE BANNER AD ---
            React.createElement(AdBanner, { key: 'subjects-ad-1' })
        );
    };
...
const handleDeleteChapter = (chapterName) => {
    ...
    handleDeleteChapter(chapter);
            ...
            // --- NATIVE BANNER AD ---
            React.createElement(AdBanner, { key: 'chapters-ad-1' })
        );
    };
...
const compressImage = (file) => {
    ...
    useEffect(() => {
        // GUEST MODE CHECK: Agar guest hai toh Firebase auth skip karo
        if (localStorage.getItem('isGuest') === 'true') {
            // Fake guest user — no Firebase auth needed
...
useEffect(() => {
    const loadFromDB = async () => {
        ...
        // ── GUEST MODE: sirf localStorage, Firebase bilkul nahi ─────────────
        if (user.isGuest || localStorage.getItem('isGuest') === 'true') {
            ...
            if (guestRaw) {
                try {
                    ...
                    const fsExam = safeData?._currentExam || '';
            ...
            setIsFetched(true);
            return; // Firebase fetch NAHI hogi
        }

        // ── LOGGED IN MODE: Normal Firebase + Cache ───────────────────────────
        if (!window.db || !window.dbFuncs) return;
...
// Auto-poll was causing unnecessary Firestore reads every 2 min per user.
    ...
    useEffect(() => {
        ...
        // ── GUEST MODE: Save to dedicated guestData key ───────────────────────
        if (user?.isGuest || localStorage.getItem('isGuest') === 'true') {
            try {
                const jsonStr = JSON.stringify(dataToSave);
                if (jsonStr.length < 2 * 1024 * 1024) {
                    localStorage.setItem('guestData', jsonStr);
                } else {
                    ...
                    localStorage.setItem('guestData', JSON.stringify({ ...rest, studyHistory: trimmed }));
                }
                localStorage.setItem('currentExam', currentExam || '');
            } catch(e) {
                if (e.name === 'QuotaExceededError') showToast('⚠️ Storage full! Old history cleared automatically.');
            }
            return; // Firebase save NAHI hogi guest ke liye
        }

        // ── LOGGED IN MODE: Normal localStorage cache ─────────────────────────
        const cacheKey = 'localDataBackup_' + user?.uid;
        const cacheTimeKey = 'localDataTime_' + user?.uid;
        try {
            const jsonStr = JSON.stringify(dataToSave);
    ...
    useEffect(() => {
        // GUEST MODE: Firebase save bilkul nahi — sirf localStorage
        if (!user || user.isGuest || localStorage.getItem('isGuest') === 'true') return;
        if (!window.dbFuncs || !window.db) return;
...
const getAnalytics = (filterClass = 'Overall') => {
    ...
    // Inject global ads once on app load
    useEffect(() => {
        injectGlobalAds();
    }, []);
